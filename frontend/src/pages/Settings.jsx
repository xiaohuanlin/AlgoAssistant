...
// Typography destructuring removed as unused
...
const Settings = () => {
    ...
    try {
      ...
      if (allConfigs.notification_config) {
        setNotificationConfig({
          notification_config: allConfigs.notification_config,
        });
      } else {
        setNotificationConfig(null);
      }
    } catch (error) {
      // Config loading errors are handled elsewhere
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGitConfigSave = async (values) => {
    try {
      ...
      // Save errors are handled by the modal component
    }
  };
            ...
            >
              <div style={{ marginBottom: 16 }}>
                <div style={{ marginBottom: 8, color: '#666' }}>
                  {t(
                    'settings.leetcodeDescription',
                    'Connect your LeetCode account to sync submissions',
                  )}
                </div>
                <Tag
                  color={getConfigStatus(leetcodeConfig?.sessionCookie).color}
                >
            ...
            >
              <div style={{ marginBottom: 16 }}>
                <div style={{ marginBottom: 8, color: '#666' }}>
                  {t(
                    'gemini.subtitle',
                    'Connect Gemini AI for intelligent code analysis',
                  )}
                </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ marginBottom: 8, color: '#666' }}>
                  {t(
                    ...
                    'Configure email, push, SMS and other notification methods',
                  )}
                </div>
                <Tag
                  color={
                    getConfigStatus(
                      notificationConfig?.notification_config?.email?.enabled ||
                        notificationConfig?.notification_config?.push
                          ?.enabled ||
                        notificationConfig?.notification_config?.sms?.enabled,
                    ).color
                  }
                >
                  {
          ...
          <Col xs={24} lg={12}>
            ...
            >
              <div style={{ marginBottom: 16 }}>
                <div style={{ marginBottom: 8, color: '#666' }}>
                  {t(
                    'accountSettings.basicInfoDesc',
                    'Manage your username, email, nickname and avatar',
                  )}
                </div>
                <Tag color="success">
                  {t('common.configured', 'Configured')}
