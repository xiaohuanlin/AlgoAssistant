...
const useTableFilters = (onFilterChange) => {
  ...
  const handleFilter = useCallback((values) => {
    ...
    Object.keys(values).forEach(key => {
      ...
      if (value !== undefined && value !== null && (value !== '' || typeof value === 'boolean')) {
        if ((key === 'dateRange' || key === 'timeRange' || key === 'created_at_range') && value?.length === 2) {
  ...
  const createAutoFilterHandler = useCallback((filterKey, debounceMs = 0) => {
    return (value) => {
        ...
        debounceTimers.current[filterKey] = setTimeout(() => {
          setFilters(currentFilters => {
            ...
            Object.keys(currentFilters).forEach(key => {
              ...
              if ((key === 'dateRange' || key === 'timeRange' || key === 'created_at_range')) {
                if (filterValue && Array.isArray(filterValue) && filterValue.length === 2) {
                  currentValues[key] = filterValue;
                }
              } else if (filterValue !== undefined && filterValue !== null && (filterValue !== '' || typeof filterValue === 'boolean')) {
                currentValues[key] = filterValue;
              }
            });
  ...
  const createFilterClearHandler = useCallback((filterKey) => {
    return () => {
      setFilters(prev => {
        const newFilters = { 
          ...prev, 
          [filterKey]: filterKey.includes('Range') ? null : (
            typeof prev[filterKey] === 'string' ? '' : undefined
          )
        };
