...
export const useDashboardData = () => {
  ...
  const fetchRecentActivity = useCallback(async (limit = 10) => {
    try {
      const response = await api.get(
        `/api/dashboard/activity/recent?limit=${limit}`,
      );
      return response.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.detail || 'Failed to load recent activity',
      );
    }
  }, []);
  ...
  const fetchErrorAnalysis = useCallback(async () => {
    try {
      ...
      throw new Error(
        error.response?.data?.detail || 'Failed to load error analysis',
      );
    }
  }, []);
  ...
  const fetchProgressTrend = useCallback(async (days = 30) => {
    try {
      const response = await api.get(
        `/api/dashboard/progress/trend?days=${days}`,
      );
      return response.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.detail || 'Failed to load progress trend',
      );
    }
  }, []);
  ...
  const fetchDashboardOverview = useCallback(async () => {
    try {
      ...
      throw new Error(
        error.response?.data?.detail || 'Failed to load dashboard overview',
      );
    }
  }, []);
      ...
      if (isMobile) {
        setTimeout(async () => {
          try {
            ...
            // Ignore errors for optional dashboard data
          }
        }, 300);
        ...
        setTimeout(async () => {
          try {
            const [categoryStats, errorAnalysis, progressTrend] =
              await Promise.all([
                fetchCategoryStats(),
                fetchErrorAnalysis(),
        ...
        const [categoryStats, recentActivity, errorAnalysis, progressTrend] =
          await Promise.all([
            fetchCategoryStats(),
