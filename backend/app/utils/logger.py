...
class LineNumberFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        while caller_frame:
            ...
            if (caller_frame.f_code.co_filename.endswith('logger.py') or 
                'logging' in caller_frame.f_code.co_filename):
                caller_frame = caller_frame.f_back
                continue
            
            # Find the actual calling location
            record.lineno = caller_frame.f_lineno
            record.filename = caller_frame.f_code.co_filename.split('/')[-1]  # Show only filename
            break
